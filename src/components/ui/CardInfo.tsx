...
import { useOneStaff } from "@/lib/hooks/useOneStaff";

type Props = {
    staffId: number | null;
    showCardInfo?: React.Dispatch<React.SetStateAction<boolean>>;
};

const CatInfo: React.FC<Props> = ({ staffId, showCardInfo }: Props) => {
    const handleBack = () => {
        if (showCardInfo) {
            showCardInfo(false);
        }
    ...
    const staff = staffId !== null ? useOneStaff(staffId) : null;
        ...
        <div className="info-panel bg-[#2e2e2e] w-full h-full absolute z-40 p-4 top-0 left-0">
                        ...
                        <div className="w-full text-center">
                            ...
                            >
                                {["Upgrade"].map((item, index) => (
                                    <div key={index} className="w-[172px]">
                                        <Button>{item}</Button>
                                    </div>
                                ))}
                            </div>
            ...
            {showStaffUpgradePanel && (
                <div className="absolute z-50 w-full h-full top-0 left-0">
                    <StaffUpgrade
                        showStaffUpgradePanel={setShowStaffUpgradePanel}
                    />
                </div>
            )}
        </div>
