...
const StaffUpgrade: React.FC<Props> = ({ showStaffUpgradePanel }) => {
    ...
    const handleChooseClick = (staffId: number) => {
        if (selectedCards.includes(staffId)) {
            setSelectedCards(selectedCards.filter((id) => id !== staffId));
        } else {
            setSelectedCards([...selectedCards, staffId]);
        }
    };
                        ...
                        >
                            {staffs.map((staff) => (
                                ...
                                >
                                    <CatCard
                                        cat={staff}
                                        active={selectedCards.includes(
                                            staff.id
                                        )}
                                        onClick={() =>
                                            handleChooseClick(staff.id)
                                        }
                                    />
                                </div>
                            ))}
